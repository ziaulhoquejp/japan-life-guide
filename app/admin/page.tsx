...
export default function AdminPage() {
  const [stats, setStats] = useState({ users: 0, schools: 0, feedback: 0, favorites: 0 })
  const [feedback, setFeedback] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    async function getData() {
      const [schools, feedbackData, favorites] = await Promise.all([
      ...
      <div style={{maxWidth:'1200px',margin:'0 auto',padding:'32px 20px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))',gap:'14px',marginBottom:'32px'}}>
          {[
            ...
            {icon:'💬',label:'Feedback Count',value:stats.feedback,color:'#2EC87A'},
            {icon:'📊',label:'Active Pages',value:'23',color:'#F0A830'},
          ].map(stat=>(
